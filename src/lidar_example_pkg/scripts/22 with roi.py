...
class PointCloudProcessor:
    ...
    def filter_roi(self, pcd, x_min=0, x_max=None, y_min=-5, y_max=5, z_min=-1, z_max=3):
        """
        ROI를 전방으로 한정하여 포인트 필터링.
        """
        points = np.asarray(pcd.points)
        mask = (
            (points[:, 0] >= x_min) &
            (x_max is None or points[:, 0] <= x_max) &
            (points[:, 1] >= y_min) &
            (points[:, 1] <= y_max) &
            (points[:, 2] >= z_min) &
            (points[:, 2] <= z_max)
    ...
    def process_pointcloud(self, pcd):
        ...
        # ROI 필터링
        pcd = self.filter_roi(pcd, x_min=0, x_max=None, y_min=-5, y_max=5, z_min=-1, z_max=3)
        ...
        with o3d.utility.VerbosityContextManager(o3d.utility.VerbosityLevel.Debug) as cm: # for debug
            labels = np.array(pcd.cluster_dbscan(eps=0.30, min_points=30, print_progress=False)) # each Point gets its label. (noise points get -1 label)
